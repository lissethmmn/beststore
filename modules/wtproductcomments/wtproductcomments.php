...
class WTProductComments extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('WT Product Comments');
		$this->description = $this->l('Allows users to post reviews and rate products on specific criteria.');
		
		$this->displayName = $this->getTranslator()->trans('Product Comments', array(), 'Modules.WTProductComments.Admin');
		$this->description = $this->getTranslator()->trans('Allows users to post reviews and rate products on specific criteria.', array(), 'Modules.WTProductComments.Admin');
	...
	protected function _postProcess()
	{
		...
		elseif (Tools::isSubmit('deletewtproductcomments'))
		{
			$id_product_comment = (int)Tools::getValue('id_product_comment');
			$comment = new ProductComment($id_product_comment);
			$comment->delete();
	...
	{
		include_once(dirname(__FILE__).'/class/ProductComment.php');
		include_once(dirname(__FILE__).'/class/ProductCommentCriterion.php');
	...
	private function _setBaseUrl()
	{
		...
		$new_tam = array("controller" => "AdminModules", "configure" => "wtproductcomments", "token" => Tools::getAdminTokenLite('AdminModules'), "controllerUri" =>"AdminModules");
		foreach ($new_tam as $k => $value)
	...
	{
		require_once(dirname(__FILE__).'/class/ProductComment.php');
		$return = null;
	...
	{
		require_once(dirname(__FILE__).'/class/ProductComment.php');
		require_once(dirname(__FILE__).'/class/ProductCommentCriterion.php');
	...
	public function hookDisplayProductListReviews($params)
	{
		...
		return $this->display(__FILE__, 'views/templates/hook/productcomments_reviews.tpl');
	}
	...
	public function hookProductTabContent($params)
	{
		...
		$this->context->smarty->assign(array(
			...
			'mediumSize' => Image::getSize(ImageType::getFormatedName('medium')),
			'nbComments' => (int)ProductComment::getCommentNumber((int)Tools::getValue('id_product')),
			'productcomments_controller_url' => $this->context->link->getModuleLink('wtproductcomments'),
