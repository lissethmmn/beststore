...
{$id_lang = Context::getContext()->language->id}
	...
	<h3 class="category-title"><span>{l s='Shop Categories' mod='wtverticalmegamenu'}</span></h3>
	...
	{$num = 0}
		...
		{$num = $num+1}
		{if $num == 8}
			<li class="level-1 more"><a href="javascript:void(0);">{l s='More categories' mod='wtverticalmegamenu'}</a></li>
		{/if}
			
			{if isset($menu.type) && $menu.type == 'CAT' && $menu.dropdown == 1}
				{$menu.sub_menu|escape:'quotes':'UTF-8' nofilter}
			{else}		
				<li class="level-1 {if $num > 7}hide_li {/if} {$menu.class|escape:'html':'UTF-8'}{if count($menu.sub_menu) > 0} parent{/if} {$menu.align_sub|escape:'html':'UTF-8'}">
		...
		{/foreach}
		{if $num > 7}
			<li class="level-1 hide"><a href="javascript:void(0);">{l s='Close categories' mod='wtverticalmegamenu'}</a></li>
		{/if}
	</ul>
	</div>
	</div>
