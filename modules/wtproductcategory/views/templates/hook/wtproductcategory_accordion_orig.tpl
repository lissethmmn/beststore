...
<div class="block-content clearfix">
	<div id="wt-prod-cat-{$cat_info.id_cat|intval}" class="row">
		...
		<div class="block-sub_cat col-sm-4 col-md-3 {if $dem%2 ==0}right{/if}">
			<div class="wt-block-title" style="background:{$cat_info.cat_color}">
			...
			<div class="cycleElementsContainer" id="sub-cycle-{$smarty.foreach.g_cat_info.iteration|escape:'html':'UTF-8'}">	
					<div id="sub-elements-{$smarty.foreach.g_cat_info.iteration|escape:'html':'UTF-8'}">
						...
						-->
						<ul class="sub-cat-ul cat-{$cat_info.id_cat}" id="sub-cat-ul-{$smarty.foreach.g_cat_info.iteration|intval}">
						{$i = 0}
						{foreach from = $cat_info.sub_cat item=sub_cat name=sub_cat_info}
							{$i = $i+1}
							<li id-cat="{$sub_cat.id_category|intval}" wt-name-module="{$name_module|escape:'html':'UTF-8'}"  id-group="{$smarty.foreach.g_cat_info.iteration|intval}" number-prod="{$cat_info.number_prod|intval}" class="item-{$i|intval} {if $i==1}active{/if}">
							<a style="" href="javascript:void(0);">{$sub_cat.name|escape}</a></li>
						...
						.sub-cat-ul.cat-{$cat_info.id_cat} li a:before 
						{
							 background: {$cat_info.cat_color};
							  border-bottom: 1px solid {$cat_info.cat_color};
						}
						...
						.wt-prod-cat #wt-prod-cat-{$cat_info.id_cat} .left-block .wt-block-title a:after
						{
							content: "";
		...
		<div class="block-products col-sm-6 col-md-6 {if $dem%2 ==0}right{/if}">
