...
class WTBlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function assign()
	{
		...
		if ($this->context->customer->isLogged())
		{
				...
				if (!count($errors))
				{
					...
					if (!count($errors))
					{
						...
						Mail::Send(
							...
							'{message}' => $this->context->link->getModuleLink('wtblockwishlist', 'view', array('token' => $wishlist->token))
							),
						...
						Tools::redirect($this->context->link->getModuleLink('wtblockwishlist', 'mywishlist'));
					}
				}
			}
			else if ($add)
			...
			Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('wtblockwishlist', 'mywishlist')));
		...
		$this->setTemplate('module:wtblockwishlist/views/templates/front/mywishlist.tpl');
	}
