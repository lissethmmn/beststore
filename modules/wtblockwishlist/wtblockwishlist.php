...
class WTBlockWishList extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->trans('WT Wishlist Block');
		$this->description = $this->trans('Adds a block containing the customer wishlists.');
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('viewwtblockwishlist') && $id = Tools::getValue('id_product'))
		...
		elseif (Tools::isSubmit('submitSettings'))
		{
			...
			if ($activated != 0 && $activated != 1)
				$this->html .= $this->trans('Activate module : Invalid choice.');
			$this->html .= $this->trans('Settings updated');
		}
	...
	public function hookDisplayProductListFunctionalButtons($params)
	{
		...
		return $this->display(__FILE__, 'views/templates/hook/wtblockwishlist_button.tpl');
	}

	public function hookHeader($params)
	{
		
		$this->context->controller->addCSS(($this->_path).'views/css/wtblockwishlist.css', 'all');
		$this->context->controller->addJS(($this->_path).'views/js/ajax-wishlist.js');

		$this->smarty->assign(array('wishlist_link' => $this->context->link->getModuleLink('wtblockwishlist', 'mywishlist')));		
		...
		if ($this->context->customer->isLogged())
		{
			...
			Media::addJsDef(array(
				...
				'mywishlist_url'	=> $this->context->link->getModuleLink('wtblockwishlist', 'mywishlist', array(), true)
			));
		...
		Media::addJsDefL('loggin_required', $this->trans('You must be logged in to manage your wishlist.', array(), 'Modules.BlockWishlist.Admin'));
		Media::addJsDefL('added_to_wishlist', $this->trans('The product was successfully added to your wishlist.', array(), 'Modules.BlockWishlist.Admin'));			
		...
		return $this->display(__FILE__, 'views/templates/hook/wtblockwishlist_top.tpl');
	}
	...
	public function hookDisplayTop($params)
	{
		if ($this->context->customer->isLogged())
		{
			...
			$this->smarty->assign(array('wishlist_products' => false, 'wishlists' => false));
		return $this->display(__FILE__, 'views/templates/hook/wtblockwishlist_nav.tpl');
	}
	...
	public function hookLeftColumn($params)
	{
		...
		//return ($this->display(__FILE__, 'views/templates/hook/wtblockwishlist.tpl'));
	}
	...
	public function hookProductActions($params)
	{
		...
		return ($this->display(__FILE__, 'views/templates/hook/wtblockwishlist-extra.tpl'));
	}
	...
	private function _displayProducts($id_wishlist)
	{
		...
		<table class="table">
			<thead>
				<tr>
					<th class="first_item" style="width:600px;">'.$this->trans('Product').'</th>
					<th class="item" style="text-align:center;width:150px;">'.$this->trans('Quantity').'</th>
					<th class="item" style="text-align:center;width:150px;">'.$this->trans('Priority').'</th>
				</tr>
			</thead>
			<tbody>';
		$priority = array($this->trans('High'), $this->trans('Medium'), $this->trans('Low'));
	...
	public function hookAdminCustomers($params)
	{
		...
		$this->html = '<h2>'.$this->trans('Wishlists').'</h2>';

		$wishlists = WishList::getByIdCustomer((int)$customer->id);
		if (!count($wishlists))
			$this->html .= $customer->lastname.' '.$customer->firstname.' '.$this->trans('No wishlist.');
		else
		{
			...
			$this->html .= '<span>'.$this->trans('Wishlist').': </span> <select name="id_wishlist" onchange="$(\'#listing\').submit();">';
	...
	{
		return $this->trans('You must be logged in to manage your wishlists.', array(), 'Modules.BlockWishlist.Admin');
	}
	...
	public function renderForm()
	{
		...
		{
			$_customers_n[$c['id_customer']]['id_customer'] = $c['id_customer'];
			$_customers_n[$c['id_customer']]['name'] = $c['firstname'].' '.$c['lastname'];
		}
		$fields_form = array(
			'form' => array(
				'legend' => array(
					'title' => $this->trans('Listing'),
					'icon' => 'icon-cogs'
				),
				'input' => array(
					array(
						...
						'name' => 'id_customer',
						'options' => array(
							'default' => array('value' => 0, 'label' => $this->trans('Choose customer')),
							'query' => $_customers_n,
							'id' => 'id_customer',
							'name' => 'name'
						),
		...
		if ($id_customer = Tools::getValue('id_customer'))
		{
			...
			$fields_form['form']['input'][] = array(
				'type' => 'select',
				'label' => $this->trans('Wishlist :'),
				'name' => 'id_wishlist',
				'options' => array(
					'default' => array('value' => 0, 'label' => $this->trans('Choose wishlist')),
					'query' => $wishlists,
					'id' => 'id_wishlist',
					'name' => 'name'
				),
	...
	public function renderList($id_wishlist)
	{
		...
		$fields_list = array(
			...
			'attributes_small' => array(
				'title' => $this->trans('Combination'),
				'type' => 'text',
			),
			'quantity' => array(
				'title' => $this->trans('Quantity'),
				'type' => 'text',
			),
			'priority' => array(
				'title' => $this->trans('Priority'),
				'type' => 'priority',
				'values' => array($this->trans('High'), $this->trans('Medium'), $this->trans('Low')),
			),
		);
		...
		$helper->title = $this->trans('Product list');
