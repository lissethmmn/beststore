...
{if $products}
	...
	{foreach from=$products item=product name=i}
		<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
			...
			<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product|intval}', {$product.id_product_attribute|escape:'html':'UTF-8'}, '0');" title="{l s='remove this product from my wishlist' mod='wtblockwishlist'}" >
				<img src="{$img_dir|escape:'html':'UTF-8'}icon/delete.gif" width="12" height="12" alt="{l s='Delete' mod='wtblockwishlist'}" class="icon" />
			</a>
		</dt>
		...
		<dd class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}" style="font-style:italic;margin:0 0 0 10px;">
			<a href="{$link->getProductLink($product.id_product, $product.link_rewrite)|escape:'html':'UTF-8'}" title="{l s='Product detail' mod='wtblockwishlist'}">{$product.attributes_small|escape:'html':'UTF-8'}</a>
		</dd>
		{/if}
	{/foreach}
	</dl>
{else}
	...
	{if isset($error) && $error}
		<dt>{l s='You must create a wishlist before adding products' mod='wtblockwishlist'}</dt>
	{else}
		<dt>{l s='No products' mod='wtblockwishlist'}</dt>
	{/if}
	</dl>
{/if}
