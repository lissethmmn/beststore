...
class CustomHtmlClass extends ObjectModel
{
	...
	public function validateController($htmlentities = true, $copy_post = false)
	{
		...
		if (!$this->{$field1} || !count($this->{$field1}) || ($this->{$field1}[$defaultLanguage] !== '0' && empty($this->{$field1}[$defaultLanguage])))
			$errors[] = '<b>Content</b> '.Tools::displayError('is empty for default language.');
